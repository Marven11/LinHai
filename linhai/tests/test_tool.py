...
class TestToolManager(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_successful_tool_call(self):
        ...
        mock_tool_call = ToolCallMessage(
            ...
            function_arguments=json.dumps({"a": 3, "b": 5})
        )

        # 模拟工具调用
        with unittest.mock.patch('linhai.tool.main.call_tool', return_value=8) as mock_call:
            ...
            self.assertEqual(result.tool_call_id, "test123")
    ...
    async def test_failed_tool_call(self):
        ...
        # 模拟工具抛出异常
        with unittest.mock.patch('linhai.tool.main.call_tool', side_effect=ValueError("Tool not found")):
            await self.manager.process_tool_call(mock_tool_call)
            
            # 验证错误消息
            result = await self.tool_output_queue.get()
            self.assertIsInstance(result, ToolErrorMessage)
            self.assertEqual(result.tool_call_id, "test456")
    ...
    async def test_manager_run_loop(self):
        ...
        mock_tool_call = ToolCallMessage(
            ...
            id="test789",
            function_name="add_numbers",
            function_arguments=json.dumps({"a": 2, "b": 3})
        )
        ...
        # 模拟工具调用
        with unittest.mock.patch('linhai.tool.main.call_tool', return_value=5) as mock_call:
            # 验证处理结果
            result = await self.tool_output_queue.get()
...
class TestToolFunctions(unittest.TestCase):
    ...
    def test_get_tools_info(self):
        ...
        self.assertEqual(tools_info[0]["function"]["description"], "Multiply two numbers")
