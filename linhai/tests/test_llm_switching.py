...
class TestLLMSwitching(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_current_llm_tool(self):
        ...
        # 验证消息中包含当前LLM信息
        self.assertTrue(any("当前使用的LLM: primary" in str(msg) for msg in self.agent.messages))
    ...
    async def test_switch_llm_tool_success(self):
        ...
        # 验证消息中包含切换成功信息
        self.assertTrue(any("已切换到LLM: secondary" in str(msg) for msg in self.agent.messages))
    ...
    async def test_switch_llm_tool_failure(self):
        ...
        # 验证消息中包含错误信息
        self.assertTrue(any("错误：LLM名称 'nonexistent' 不存在" in str(msg) for msg in self.agent.messages))
        self.assertTrue(any("可用的LLM包括: primary, secondary" in str(msg) for msg in self.agent.messages))
