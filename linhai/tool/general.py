...
@global_tools.register_tool(
    ...
    args={
        "group_chat": ToolArgInfo(
            desc="GroupChat实例，用于获取上下文", type="GroupChat"
        ),
    },
    required_args=["group_chat"],
)
async def list_llm(group_chat: GroupChat) -> str:
    """列出所有可用的LLM。"""
    from linhai.agent import Agent

    agent = group_chat.get_member_typechecked("agent", Agent)
    # agent is guaranteed to be not None due to type checking
