...
class Agent:
    ...
    def get_threshold_info(self) -> ThresholdInfo | None:
        ...
        if token_info.is_dirty or token_info.current_token_usage is None:
            return None
        ...
        used_tokens = token_info.current_token_usage.total_tokens
    ...
    async def generate_response(self) -> ParsedAnswer:
        ...
        if current_llm.get_native_toolcall_format():
            parsed_toolcalls = await parsed_answer.get_native_toolcalls()
            if parsed_toolcalls:
            ...
            await self.lifecycle.after_message_generation.trigger(
                parsed_answer, parsed_toolcalls or []
            )
        else:
