...
def create_agent_build_context(
    ...
    return {
        ...
        "claw_enabled": build_args.get("claw_enabled", False) or agent_config.claw,
...
async def create_agent_from_context(
    ...
    if tool_config.file_operation_rules:
        ...
        FileOperationPermissionPlugin(context["registry"], tool_config).register(
            agent.lifecycle
        )
    ...
    for llm_config in context["llms"]:
        if llm_config.type == "anthropic":
            llm = AnthropicLanguageModel(
                ...
                native_toolcall_format=llm_config.native_toolcall_format,
            )
        else:
            llm = OpenAi(
                registry=context["registry"],
...
async def _create_pinned_messages(context: "AgentBuildContext") -> list[Message]:
    ...
    if context["file"]:
        from linhai.multimodal import load_image_as_message
        ...
        for file_path in context["file"]:
            if file_path.suffix.lower() in _IMAGE_EXTENSIONS:
                pinned_messages.append(
                    await load_image_as_message(
                        str(file_path), context["registry"], "raw"
                    )
                )
            else:
...
def _register_default_plugins(lifecycle):
    ...
    plugins = [
        ...
        HackerNewsPlugin(lifecycle.registry),
        CsdnWarningPlugin(lifecycle.registry),
        ...
        StdioCommandCheckerPlugin(lifecycle.registry),
        PkillCheckerPlugin(lifecycle.registry),
    ]
