...
async def terminal_create(
    ...
    if not await check_tmux_available(host):
        return FailedToolResult(content="远程机器没有安装tmux，无法创建终端")
    ...
    if rc != 0:
        return FailedToolResult(content=f"创建终端失败: {stderr or stdout}")
...
async def terminal_send_keys(
    ...
    if terminal_id not in _terminals:
        return FailedToolResult(content=f"未找到终端 {terminal_id}")
    ...
    for key in keys:
        ...
        if rc != 0:
            return FailedToolResult(content=f"发送按键失败: {stderr}")

    return SuccessfulToolResult(content=f"已发送按键: {keys}")
...
async def terminal_send_string(
    ...
    rc, _, stderr = await host.execute_raw(cmd)
    if rc != 0:
        return FailedToolResult(content=f"发送字符串失败: {stderr}")

    if with_enter:
        cmd = f"tmux send-keys -t {shlex.quote(session)} Enter"
        rc, _, stderr = await host.execute_raw(cmd)
        if rc != 0:
            return FailedToolResult(content=f"发送回车失败: {stderr}")

    await asyncio.sleep(wait_seconds)
    screen = await terminal_read_screen(host, terminal_id)
    if isinstance(screen, FailedToolResult):
        return screen
    return SuccessfulToolResult(
        content=f"已发送: {string}, 当前内容:\n" + screen.content
    )
...
async def terminal_read_screen(
    ...
    rc, stdout, stderr = await host.execute_raw(cmd)
    if rc != 0:
        return FailedToolResult(content=f"读取屏幕失败: {stderr}")
...
async def terminal_close(
    ...
    return SuccessfulToolResult(content=f"已关闭终端 {terminal_id}")
...
async def get_terminals(
    ...
    if not _terminals:
        return SuccessfulToolResult(content="<<terminals>>没有活动的终端<<terminals>>")
    ...
    for term_id in _terminals:
        ...
        screen = (
            ...
            if isinstance(screen_result, SuccessfulToolResult)
            else "无法获取屏幕内容"
        )
        lines.append(
