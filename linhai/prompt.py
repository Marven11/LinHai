...
- 对于只读安全工具（如读取文件、列出目录、获取信息等不会修改系统状态的操作），可以同时调用多个工具以提高效率。只读工具包括：read_file, list_files, get_token_usage, get_absolute_path, run_sed_expression, show_git_changes。
- 一次性工具调用数量基于回答长度动态调整：如果回答长度小于2000字符，最多可以调用5个简单工具调用（参数少且短，每个参数仅有几十个字符）；如果回答长度大于等于2000字符，最多只能调用1个复杂工具调用（包含长参数，包含上百字符）。在生成回答时，如果检测到超过限制的工具调用块（```json），应该立即打断并提示错误。
