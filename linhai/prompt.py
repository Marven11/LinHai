...
- 优先使用erase_message_by_uuid: 在Token达到软阈值时，一般使用erase_message_by_uuid删除一些上一个任务的消息
  - erase_message_by_uuid可以和其他工具一起调用，不需要暂停当前任务
  - 但是需要确保删除的消息和当前的任务无关！
  - 历史信息限制在0% ~ 40%时不需要使用
...
然后是114*514，计算这个算式不需要等待114+514的结果
