...
- [ ] 将load_image的参数改为image_filepath并在linhai/plugin/tool_call_managers.py编写插件在参数以`https://`或者`http://`等开头时警告
  - 问题：agent有时会误以为支持传入url
  - 需要编写良好的测试
- [ ] 添加list_llm工具让agent可以手动列出并查看当前的llm
  - 仅展示每个llm的名字, model和token_limit
  - 为了架构合理，需要为LanguageModel添加一个获取描述的接口
...
- [ ] PlanningStatusReminderPlugin有bug:
  - 在没有调用任何工具但是处于红灯状态时没有移除提示
  - 期望
    - 在红灯状态：不增加计数器，提示不应该存在
    - 没有调用任何工具：此时agent暂时弄错了工具调用的格式或者正在回复用户，不增加计数器，根据计数器判断提示是否应该存在
    - 调用了工具但是没有修改.md，说明agent正在工作或者忘记修改.md，增加计数器，根据计数器判断提示是否应该存在
  - 测试
    - 没有调用任何工具时，计数器不增加
    - 不处于红灯状态但是调用了任何工具但是没有修改.md时，增加对应计数器
    - 在红灯状态但是没有调用任何工具时，提示应该不存在
    - 在脱离红灯状态但是没有调用任何工具时，根据计数器状态判断提示词是否应该存在
