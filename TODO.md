...
- [ ] 改进linhai/cli/messages_list.py性能
  ...
  - 主要改进：在没有向上滚动时unmount上方看不见的消息，在用户向上滚动时重新mount回来
  - 设计
    - 在自动滚动开启时：
      - 如果消息多于20条且消息列表widget高度高于1000则unmount最上方的一个widget,按照原有顺序保存到列表中
    - 在自动滚动关闭时：
      - 自动滚动关闭说明用户有可能向上滚动
      - 如果当前滚动位置距离顶部少于500则按照顺序恢复一个消息，mount到上面（根据文档使用before参数）
    - 无论有没有unmount/mount消息都sleep 0.05秒等待界面刷新
  - 以上设计难以验证，需要严格按照官网文档编写测试
    - 在有大量消息且开启自动滚动时上方有多条消息被unmount
    - 滚动到上方时消息被逐个恢复且顺序和被unmount前保持相同
- [ ] 重构cli提升速度
