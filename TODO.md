...
- [ ] 当前planning检查连续x次未修改STATUS.md/TODOLIST.md的逻辑是错的
  - 期望行为
    - 注册AfterMessageGenerationCallback
    - 连续**x条包含工具调用的消息**没有修改.md则提示，如果有一条消息修改了对应.md则清零对应计数器
    - 只有在当前有工具调用且工具调用中没有修改对应.md时增加计数器
  - 添加测试
    - 如果一条消息有至少x个工具调用都没有修改对应文件则不添加提醒
    - 如果连续x条消息都没有工具调用则不添加提醒
    - 如果连续x条消息都调用了工具，消息数量超出限制，但是都没有修改对应的.md，则添加对应提醒
- [ ] 改进删除大消息的逻辑
  - 当前问题: remove_message直接删除了消息
  - 设计
    - 完全删除这个函数
    - 改成replace_message函数
    - 在删除大消息时不直接删除，而是改为占位符消息
      - RuntimeMessage "当前消息已经被遗忘，转储到xxx"
    - 以上逻辑需要在转储消息前先创建文件，你需要先保存大消息，得到文件路径后再替换消息
