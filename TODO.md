...
- [ ] 当前根据“一分钟内是否调用过上下文清理工具”判断是否需要禁止重新调用的逻辑过于脆弱
  ...
  - 编写测试
    ...
    - 在清理上下文后红绿灯状态变为“失效”
- [ ] 添加命令行参数--afk: 关闭#LINHAI_WAITING_USER的功能
  - 当用户指定了--afk参数时，注册一个新的插件
  - 这个插件
    - 在agent启动时添加消息提示“用户使用--afk参数启动，这说明用户离开了电脑且禁止你暂停等待用户，当前通过#LINHAI_WAITING_USER等待用户的功能已失效”
    - 在agent等待用户时将agent的状态改为working阻止agent等待用户
