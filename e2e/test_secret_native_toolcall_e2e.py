...
def test_secret_native_toolcall_mask_and_read_e2e():
    secret_uuid = _generate_uuid()
    plaintext_uuid = _generate_uuid()

    secret_path = Path(tempfile.mktemp(suffix=".toml", prefix="test_secret_native_"))
    config_path = Path(tempfile.mktemp(suffix=".toml", prefix="test_config_native_"))
    _create_secret_config(secret_path, secret_uuid)
    _create_test_config(config_path, str(secret_path))

    test_file = Path(f"/tmp/test_secret_native_{os.getpid()}.txt")
    test_file.write_text(f"secret_id={secret_uuid}\nplain_id={plaintext_uuid}")

    session_name = f"test_secret_native_{os.getpid()}"
    ...
    try:
        subprocess.run(
            [
                ...
                "-m",
                (
                    f"Read the file at {test_file} using call_with_secret tool "
