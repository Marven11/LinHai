...
def test_anthropic_e2e():
    test_uuid = _generate_uuid()
    config_path = Path(tempfile.mktemp(suffix=".toml", prefix="anthropic_e2e_"))
    _create_test_config(config_path)
    test_file = Path(f"/tmp/anthropic_e2e_{os.getpid()}.txt")
    test_file.write_text(f"The secret UUID for verification is: {test_uuid}\n")
    session_name = f"anthropic_e2e_{os.getpid()}"
