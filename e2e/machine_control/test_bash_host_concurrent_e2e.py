...
from linhai.tool.base import SuccessfulToolResult, FileContentToolResult
from tests.test_helpers import _AsyncioProcessAdapter


class TestBashHostConcurrentE2E(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_concurrent_read_with_ping(self):
        control, adapter, process = await self._create_control()
        connected = await control.connect(adapter)
        self.assertTrue(connected, "连接bash失败")

        large_file = "linhai/prompt.py"
            ...
            while time.monotonic() - start < 60.0:
                try:
                    ...
                    if isinstance(result, FileContentToolResult):
                        ...
                        read_errors.append(f"读取失败: {result}")
                except Exception as e:
                    read_errors.append(f"读取异常: {e}")
