...
class TestUnnecessarySedReadPlugin(unittest.TestCase):
    ...
    @patch("linhai.agent.plugin.Path")
    @patch("builtins.open", new_callable=mock_open, read_data=b"line1\nline2\nline3\n")
    def test_duplicate_file_read_plugin_allows_different_content(self, mock_open, mock_path):
        """测试读取相同文件但内容不同应允许。"""
        from linhai.agent.plugin import DuplicateFileReadPlugin
        plugin = DuplicateFileReadPlugin(self.group_chat)
        ...
        from linhai.agent.base import FileContentMessage as FCM
        old_file_content = FCM(absolute_path, "old line1\nold line2\nold line3\n")
    ...
    @patch("linhai.agent.plugin.Path")
    @patch("builtins.open", new_callable=mock_open, read_data=b"line1\nline2\nline3\n")
    def test_duplicate_file_read_plugin_ignores_on_non_master_host(self, mock_open, mock_path):
        ...
        # 模拟消息历史中包含FileContentMessage
        from linhai.agent.base import FileContentMessage as FCM
        old_file_content = FCM(absolute_path, "line1\nline2\nline3\n")
        self.agent.message_processor.get_messages.return_value = [old_file_content]
    ...
    @patch("linhai.agent.plugin.Path")
    @patch("builtins.open", new_callable=mock_open, read_data=b"line1\nline2\nline3\n")
    def test_duplicate_file_read_plugin_handles_multiple_messages_latest_same(self, mock_open, mock_path):
        """测试多个历史消息，最新相同应拦截。"""
        from linhai.agent.plugin import DuplicateFileReadPlugin
        plugin = DuplicateFileReadPlugin(self.group_chat)
        ...
        from linhai.agent.base import FileContentMessage as FCM
        old_content1 = FCM(absolute_path, "old content")
        old_content2 = FCM(absolute_path, "different content")
        latest_content = FCM(absolute_path, "line1\nline2\nline3\n")
        self.agent.message_processor.get_messages.return_value = [old_content1, old_content2, latest_content]
    ...
    @patch("linhai.agent.plugin.Path")
    @patch("builtins.open", new_callable=mock_open, read_data=b"line1\nline2\nline3\n")
    def test_duplicate_file_read_plugin_handles_multiple_messages_latest_different(self, mock_open, mock_path):
        """测试多个历史消息，最新不同应允许。"""
        from linhai.agent.plugin import DuplicateFileReadPlugin
        plugin = DuplicateFileReadPlugin(self.group_chat)
        ...
        from linhai.agent.base import FileContentMessage as FCM
        old_content1 = FCM(absolute_path, "line1\nline2\nline3\n")
        old_content2 = FCM(absolute_path, "line1\nline2\nline3\n")
        latest_content = FCM(absolute_path, "different content")
        self.agent.message_processor.get_messages.return_value = [old_content1, old_content2, latest_content]
    ...
    @patch("linhai.agent.plugin.Path")
    @patch("builtins.open", new_callable=mock_open, read_data=b"line1\nline2\nline3\n")
    def test_duplicate_file_read_plugin_handles_resolve_error_current_path(self, mock_open, mock_path):
        """测试当前文件路径解析失败时返回None。"""
        from linhai.agent.plugin import DuplicateFileReadPlugin
        plugin = DuplicateFileReadPlugin(self.group_chat)
        ...
        # 模拟read_file工具调用
        from linhai.agent.base import FileContentMessage as FCM
        new_file_content = FCM("/some/path/test.py", "line1\nline2\nline3\n")
    ...
    @patch("linhai.agent.plugin.Path")
    @patch("builtins.open", new_callable=mock_open, read_data=b"line1\nline2\nline3\n")
    def test_duplicate_file_read_plugin_handles_resolve_error_historical_path(self, mock_open, mock_path):
        ...
        self.agent.message_processor.get_messages.return_value = [bad_message, good_message]
        
        # 模拟read_file工具调用，返回与好消息相同的内容
        new_file_content = FCM("/absolute/path/test.py", "line1\nline2\nline3\n")
    ...
    def test_duplicate_file_read_plugin_returns_none_on_non_file_content_message(self):
        ...
        result = asyncio.run(
            plugin._after_tool_call(
                self.agent, tool_call, "just a string result", True
            )
        )
    ...
    @patch("linhai.agent.plugin.Path")
    def test_duplicate_file_read_plugin_returns_none_on_value_error_resolve(self, mock_path):
        ...
        from linhai.agent.base import FileContentMessage as FCM
        new_file_content = FCM("/some/file.txt", "content")
