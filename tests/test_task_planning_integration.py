...
class TestTaskPlanningIntegration(unittest.IsolatedAsyncioTestCase):
    ...
    async def asyncSetUp(self):
        ...
        self.agent.lifecycle._plugins = [
            p
            for p in self.agent.lifecycle._plugins
            if not isinstance(p, TaskPlanningEnforcementPlugin)
        ]
        # 创建新的插件实例
        ...
        self.agent.lifecycle.register_after_message_generation(
            self.planning_plugin.after_message_generation
        )
        self.agent.lifecycle.register_after_token_generation(
            self.planning_plugin.after_token_generation
        )
    ...
    async def test_task_planning_enabled(self):
        ...
        mock_answer = MockAnswer([MockAnswerToken(content=response_with_planning)])
        ...
        planning_plugins = [
            p
            for p in self.agent.lifecycle._plugins
            if isinstance(p, TaskPlanningEnforcementPlugin)
        ]
        # 如果没有找到插件，可能是注册问题
    ...
    async def test_task_planning_missing(self):
        ...
        mock_answer = MockAnswer([MockAnswerToken(content=response_without_planning)])
        self.mock_llm.answer_stream.return_value = mock_answer
    ...
    async def test_task_planning_interruption(self):
        ...
        with patch.object(
            self.agent, "interrupt", new_callable=AsyncMock
        ) as mock_interrupt:
            # 调用after_message_generation
            await plugin.after_message_generation(
                AsyncMock(), response_without_planning, [{"name": "switch_llm"}]
            )
