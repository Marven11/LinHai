...
class TestSecretExceptionLeak(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.mock_group_chat.get_member_typechecked.return_value = self.mock_tool_manager
        
        # 创建secret插件
        self.secret_plugin = SecretInterceptorPlugin(self.mock_group_chat, self.secrets_dict)
        ...
        def failing_tool_with_secret(param: str):
            ...
            raise ValueError(f"Tool failed with parameter: {param} and secret: super-secret-password-123")
        ...
        self.mock_tool_manager.has_tool = lambda name: name in ["failing_tool_with_secret", "failing_tool_without_secret"]
    ...
    async def test_tool_exception_leaks_secret_without_with_secret(self):
        ...
        self.assertIn("super-secret-password-123", result_content,
        ...
        self.assertIsNotNone(plugin_result, 
                           "secret插件应该拦截包含secret的结果（没有指定with_secret）")
        plugin_result_str = str(plugin_result)
        self.assertIn("已拦截", plugin_result_str,
                     "应该显示拦截消息")
        self.assertNotIn("super-secret-password-123", plugin_result_str,
                        "拦截后的消息不应该包含secret值")
    ...
    async def test_tool_exception_masked_with_with_secret(self):
        ...
        # 验证secret插件是否正确处理了结果
        self.assertIsNotNone(plugin_result, 
                           "secret插件应该处理包含secret的结果（指定了with_secret）")
        plugin_result_str = str(plugin_result)
        
        # 结果应该被掩码，包含<<masked>>标记
        self.assertIn("<<masked>>", plugin_result_str,
                     "应该显示掩码标记")
        # secret值应该被替换为占位符
        self.assertIn("<$TEST_SECRET$>", plugin_result_str,
                     "secret值应该被替换为占位符")
        self.assertNotIn("super-secret-password-123", plugin_result_str,
    ...
    async def test_tool_exception_without_secret_not_intercepted(self):
        ...
        # 验证异常信息中不包含secret值
        self.assertNotIn("super-secret-password-123", result_content,
                        "异常信息不应该包含secret值")
        self.assertNotIn("another-secret-value", result_content,
                        "异常信息不应该包含其他secret值")
    ...
    def test_run_tests(self):
        ...
        try:
            loop.run_until_complete(self.test_tool_exception_leaks_secret_without_with_secret())
            loop.run_until_complete(self.test_tool_exception_masked_with_with_secret())
            loop.run_until_complete(self.test_tool_exception_without_secret_not_intercepted())
        finally:
            loop.close()
