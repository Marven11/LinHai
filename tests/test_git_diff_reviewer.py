...
class TestGitDiffReviewPlugin(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.temp_code_style_file = tempfile.NamedTemporaryFile(
            mode="w", suffix=".md", delete=False
        )
        self.temp_code_style_file.write("# 代码风格要求\n")
        self.temp_code_style_file.close()
        # 将checklist设置为Path对象
        self.cli_args.checklist = Path(self.temp_code_style_file.name)
    ...
    def _setup_git_mocks(
        self, mock_exists, mock_run, git_diff=None, ls_files_output=None
    ):
        """设置git相关命令的mock。"""
        ...
        mock_run.side_effect = [
            ...
            Mock(
                stdout=ls_files_output, returncode=0
    ...
    @patch("subprocess.run")
    @patch("os.path.exists")
    def test_before_waiting_user_with_diff(
        self, mock_exists, mock_run, mock_create_task
    ):
        """测试有git diff时启动审查。"""
        mock_exists.return_value = True
        mock_run.side_effect = [
    ...
    @patch("subprocess.run")
    @patch("os.path.exists")
    def test_before_waiting_user_no_change(
        self, mock_exists, mock_run, mock_create_task
    ):
    ...
    @patch("subprocess.run")
    @patch("os.path.exists")
    def test_before_waiting_user_no_change_ui_message(
        self, mock_exists, mock_run, mock_create_task
    ):
        """测试没有变化时发送UI消息。"""
        self._setup_git_mocks(mock_exists, mock_run)
        ...
        with patch.object(self.plugin.group_chat, "send_if_exists") as mock_send:
            ...
            self.assertEqual(
                call_args[0][1].content,
                "未触发SubAgent审核：检测到与上一次完全相同的git更改，无需重复审查",
            )

    @patch("asyncio.create_task")
    @patch("subprocess.run")
    @patch("os.path.exists")
    def test_before_waiting_user_with_deleted_files(
        self, mock_exists, mock_run, mock_create_task
    ):
        """测试有删除文件时启动审查。"""
        mock_exists.return_value = True
    ...
    @patch("subprocess.run")
    @patch("os.path.exists")
    def test_before_waiting_user_with_staged_deleted_files(
        self, mock_exists, mock_run, mock_create_task
    ):
        """测试有暂存区删除文件时启动审查。"""
        mock_exists.return_value = True
    ...
    @patch("subprocess.run")
    @patch("os.path.exists")
    def test_before_waiting_user_git_diff_changed(
        self, mock_exists, mock_run, mock_create_task
    ):
        """测试git diff变化时重新启动审查。"""
        self._setup_git_mocks(mock_exists, mock_run)
    ...
    @patch("subprocess.run")
    @patch("os.path.exists")
    def test_before_waiting_user_new_files_changed(
        self, mock_exists, mock_run, mock_create_task
    ):
        """测试新增文件内容变化时重新启动审查。"""
        self._setup_git_mocks(mock_exists, mock_run)
    ...
    @patch("subprocess.run")
    @patch("os.path.exists")
    def test_before_waiting_user_deleted_files_changed(
        self, mock_exists, mock_run, mock_create_task
    ):
        """测试删除文件列表变化时重新启动审查。"""
        self._setup_git_mocks(mock_exists, mock_run)
    ...
    @patch("builtins.open")
    @patch("os.path.isdir")
    def test_read_single_file_content_large_file(
        self, mock_isdir, mock_open, mock_getsize
    ):
