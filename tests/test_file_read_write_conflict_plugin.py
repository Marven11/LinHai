...
class TestFileReadWriteConflictPlugin(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_read_then_write_same_file_should_warn(self):
        ...
        # 调用after_tool_call处理读取
        result = await self.plugin.after_tool_call(
            MagicMock(), read_tool_call, read_result, True
        )
        self.assertIsNone(result)  # 读取文件不应该返回警告
        ...
        # 调用after_tool_call处理写入
        result = await self.plugin.after_tool_call(
            MagicMock(), write_tool_call, write_result, True
        )
    ...
    async def test_failed_tool_call_should_be_ignored(self):
        ...
        # 调用after_tool_call，但success=False
        result = await self.plugin.after_tool_call(
            MagicMock(), read_tool_call, read_result, False
        )
        self.assertIsNone(result)  # 失败的工具调用应该被忽略
