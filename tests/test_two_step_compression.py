...
class TestTwoStepCompressionBasic(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_step1_creates_range_clean_id(self):
        ...
        def mock_get_members(member_type, member_class=None):
        ...
        mock_agent.message_processor.filter_messages = AsyncMock(side_effect=mock_filter_messages)
    ...
    async def test_step2_valid_range_clean_id(self):
        ...
        def mock_get_members(member_type, member_class=None):
            if member_type == "agent":
                ...
                mock_manager = MagicMock(spec=RangeCleanManager)
                mock_manager.get_clean_info = MagicMock(return_value=RangeCleanInfo(
    ...
    async def test_messages_list_summerize_message_format(self):
        ...
        self.assertIn("[消息列表已无效，ID: rangeclean_12345]", invalid_message["content"])
    ...
    async def test_step2_user_message_protection_summary(self):
        ...
        self.assertTrue(user_message_summary_found, 
                       "No user message summary was created")
