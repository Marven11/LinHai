    ...
    async def test_plugin_registered_when_allowed_commands_present(self):
        ...
        with patch("linhai.agent.create._create_llm_instances") as mock_create_llms, \
            ...
            with patch("linhai.agent.create.Agent", return_value=mock_agent):
                ...
                # 验证machine_control.register_plugin被调用
                mock_machine_control.register_plugin.assert_called_once_with(mock_agent.lifecycle)
                
                # 验证tool_manager.register_lifecycle被调用
                mock_tool_manager.register_lifecycle.assert_called_once()
                
                # 验证CommandWhitelistPlugin被注册（通过lifecycle.register调用）
                # 由于我们无法直接检查，但可以确认流程执行完毕
    ...
    async def test_plugin_not_registered_when_no_allowed_commands(self):
        ...
        with patch("linhai.agent.create._create_llm_instances") as mock_create_llms, \
            ...
            with patch("linhai.agent.create.Agent", return_value=mock_agent):
                ...
                # 验证CommandWhitelistPlugin没有被注册（create_subagent仍应被调用）
                mock_create_subagent.assert_called_once()
