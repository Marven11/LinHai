...
def _resolved(filepath: str) -> str:
...
class TestSedFragmentedReadPlugin(unittest.TestCase):
    ...
    def test_large_token_resets_count(self, mock_tokens):
        self.plugin._count[_resolved("/tmp/test.py")] = 2
        result = asyncio.run(
            self.plugin.after_toolcall(
                "read_file_with_sed",
                0,
        ...
        self.assertIsNone(result)
        self.assertEqual(self.plugin._count.get(_resolved("/tmp/test.py"), 0), 0)
    ...
    def test_second_read_with_overlap_count_1_no_trigger(self, mock_tokens):
        ...
        self.assertEqual(self.plugin._count.get(_resolved("/tmp/test.py"), 0), 1)
    ...
    def test_third_read_with_overlap_triggers(self, mock_tokens):
        ...
        self.assertIsNone(result)
        self.assertEqual(self.plugin._count.get(_resolved("/tmp/test.py"), 0), 2)
    ...
    def test_cleanup_removes_old_records(self, mock_tokens):
        ...
        self.plugin._records[_resolved("/tmp/test.py")] = [
            ({"line1", "line2"}, old_time)
        ]
        self.plugin._count[_resolved("/tmp/test.py")] = 5
        asyncio.run(
            self.plugin.after_toolcall(
                "read_file_with_sed",
                0,
